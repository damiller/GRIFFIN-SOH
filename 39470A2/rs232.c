              ...
              int mode)
{
  ...
  struct {
    ...
    {115200, B115200}, {230400, B230400}, {0,B0}
  };
...
int rs232_gets_wait(int hDev, char *str, int size, int end_char)
{
  ...
  for (i=0;i<50;i++)
  {
	...
	nbytes = 0;
      ...
      {
        char ctemp[256];
	//sprintf(ctemp,"fnctl(): read error %d (%s), nbytes=%d",
	//  errno, strerror(errno),nbytes);
	// printf(ctemp);
	cm_msg(MINFO,"rs232_gets_wait","fnctl(): read error",0);
	return 0;
      }
    }
