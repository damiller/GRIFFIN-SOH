...
static int odbMkdir(const char* str, HNDLE* handlep)
{
  int status = db_find_key(hDB, 0, str, handlep);

  if (status == DB_NO_KEY)
    {
      status = db_create_key(hDB, 0, str, TID_KEY);
      if (status == DB_SUCCESS)
	status = db_find_key(hDB, 0, str, handlep);
    }
  
  if (status != SUCCESS)
    {
      cm_msg(MERROR, frontend_name, "Cannot find or create \'%s\', status %d, exiting", str, status);
      return -1;
    }
